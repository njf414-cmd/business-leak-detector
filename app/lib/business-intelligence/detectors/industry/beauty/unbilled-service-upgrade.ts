...
function isEarned(value: unknown): boolean {
  return ["completed","provided","performed","upgraded"].includes(normalizeText(value));
}
...
export const unbilledBeautyServiceUpgradeDetector: BusinessLeakDetector = {
  id: "beauty.unbilled-service-upgrade",
  name: "Unbilled Service Upgrade",
  description: "Detects documented unbilled service upgrade opportunities where revenue is due or earned but has not been properly billed or collected.",
  scope: "industry",
  industries: ["beauty"],
  requirements: {
    optionalFields: ["Customer Name","Project Name","Service Upgrade","Service Upgrade Amount","Service Upgrade Status","Service Upgrade Billing Status","Service Date"],
  },
  supports(profile) {
  ...
  async detect(context: DetectorContext): Promise<DetectorResult> {
    ...
    context.rows.forEach((row, rowIndex) => {
      ...
      leaks.push({
        detectorId: "beauty.unbilled-service-upgrade",
        ...
        metadata: {
          ...
          detectionReason: "beauty_service_upgrade_not_billed",
        },
      });
    });
