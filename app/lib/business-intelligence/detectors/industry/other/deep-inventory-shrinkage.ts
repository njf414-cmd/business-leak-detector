...
function matchesRule(row: Record<string, unknown>): boolean {
  const left = parseNumber(
    row["Expected Inventory"]
  );

  const right = parseNumber(
    row["Actual Inventory"]
  );
...
export const deepInventoryShrinkageDetector: BusinessLeakDetector = {
  id: "deep.inventory-shrinkage",
  name: "Inventory Shrinkage",
  description: "Detects inventory shrinkage using documented business data and a comparison rule.",
  scope: "universal",
  industries: [],
  requirements: {
    requiredFields: ["Revenue Impact", "Expected Inventory", "Actual Inventory"],
  ...
  async detect(
    ...
    context.rows.forEach(
      (
        ...
        leaks.push({
          detectorId:
            "deep.inventory-shrinkage",

          leakType:
            "Inventory Shrinkage",

          title:
            `${customerName} has a inventory shrinkage`,
